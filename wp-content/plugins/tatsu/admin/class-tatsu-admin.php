...
class Tatsu_Admin {
	...
	public function tatsu_admin_notice(){
		//Spyro Theme Notice
		global $pagenow;
		if(isset($pagenow) && in_array( $pagenow,[ 'index.php', 'themes.php', 'plugins.php' ])) {
			$tatsu_admin_dismiss_notices = get_option('tatsu_admin_dismiss_notices', array());
			if(is_array($tatsu_admin_dismiss_notices)){
				echo in_array('tatsu-typehub-colorhub-info-notice', $tatsu_admin_dismiss_notices)?'':'<div id="tatsu-typehub-colorhub-info-notice" class="notice tatsu-admin-notice notice-info is-dismissible">
				<h3>'.esc_html__( 'Typehub and Colorhub are now included in Tatsu', 'tatsu' ).'</h3>
				<p>'.esc_html__( 'Typehub and Colorhub settings are avilable inside Tatsu menu. Please deactivate Typehub and Colorhub plugins.', 'tatsu' ).'</p></div>';

				echo in_array('tatsu-spyro-info-notice', $tatsu_admin_dismiss_notices)?'':'
				<div id="tatsu-spyro-info-notice" class="notice tatsu-admin-notice notice-info is-dismissible">
